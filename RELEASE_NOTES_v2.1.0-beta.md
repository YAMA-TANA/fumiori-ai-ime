...
- `Yamatana-AI-IME-v2.1.0-beta-hotfix-20260919-r7.zip`（推奨。非同期プリフェッチ＋Mozcプロセスのロック解除待ち／リトライ対応）
- `SHA256SUMS.txt`
...
- r7 では payload の SHA-256 が既に一致している場合は上書きをスキップし、ロック中の同一ファイルを無理に置換しません。差分がある場合だけ `mozc_broker`、`mozc_server`、`mozc_renderer` を終了してリトライします。常駐監視で再生成される `mozc_cache_service` は対象外です。

### Existing installation

既存 MSI を使っている場合は、次の hotfix ZIP を展開し、正規パスまたは互換パスのスクリプトを管理者 PowerShell から実行してください。

- 推奨: `Yamatana-AI-IME-v2.1.0-beta-hotfix-20260919-r7.zip`
- SHA-256: `C16C8C1FB6C1FA9F140F3635AD617048B92747972664FB46D81110A5184A0F4B`
- 正規スクリプト: `scripts\apply_hotfix.ps1`
- 互換スクリプト: `scripts\apply\_hotfix.ps1`
- 旧 20260918 / 20260919 / r2 / r3 / r4 / r5 / r6 ZIP は使用しないでください。

実行コマンド:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\Users\lotof\Videos\research\live2d\yamatana-ai-ime\build\hotfix-20260919-r7\scripts\apply_hotfix.ps1"
```
