...
- `Yamatana-AI-IME-v2.1.0-beta-hotfix-20260919-r9.zip`（推奨。非同期プリフェッチ＋Mozcプロセスのロック解除待ち／リトライ対応）
- `SHA256SUMS.txt`
...
- r9 では `taskkill.exe` を別プロセスとして実行し、「対象プロセスなし」の stderr を PowerShell の致命的エラーにしません。先に `mozc_broker`、`mozc_server`、`mozc_renderer` を終了してから payload の SHA-256 を比較します。一致していれば上書きをスキップし、差分がある場合だけコピーをリトライします。

### Existing installation

既存 MSI を使っている場合は、次の hotfix ZIP を展開し、正規パスまたは互換パスのスクリプトを管理者 PowerShell から実行してください。

- 推奨: `Yamatana-AI-IME-v2.1.0-beta-hotfix-20260919-r9.zip`
- SHA-256: `2FD9081FC90DA849EA0872927EC683FB0F858BEA090418D130C7075F39292874`
- 正規スクリプト: `scripts\apply_hotfix.ps1`
- 互換スクリプト: `scripts\apply\_hotfix.ps1`
- 旧 20260918 / 20260919 / r2 / r3 / r4 / r5 / r6 / r7 / r8 ZIP は使用しないでください。

実行コマンド:

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\Users\lotof\Videos\research\live2d\yamatana-ai-ime\build\hotfix-20260919-r9\scripts\apply_hotfix.ps1"
```
